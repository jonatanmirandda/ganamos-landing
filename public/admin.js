/* =========================================================
 * Panel admin · login HTTP + WebSockets opcionales
 * El login YA NO depende de WebSockets — usa fetch() normal.
 * Si Socket.io anda, agrega tiempo real. Si no, recargás manual.
 * =======================================================*/
(() => {
  ...
  loginForm.addEventListener('submit', async (e) => {
    ...
    try {
      // Timeout de 60 segundos por si Render está despertando del sueño
      ...
      if (!resp.ok || !data.ok) {
        ...
        loginErr.textContent = data.error || 'Usuario o contraseña incorrectos';
        return;
      }
      ...
      if (err.name === 'AbortError') {
        loginErr.textContent = 'El servidor tardó mucho en responder. Probá de nuevo en unos segundos.';
      } else {
        loginErr.textContent = 'No se pudo conectar: ' + (err.message || 'error de red');
      }
    }
  });
  ...
  function connectSocket() {
    try {
      if (typeof io === 'undefined') { setConn(false, 'sin tiempo real'); return; }
      ...
      socket.on('connect_error', () => setConn(false, 'sin tiempo real'));

      socket.on('chat:updated', () => { reloadChats(); });
  ...
  function setConn(live, label) {
    ...
    connStatus.querySelector('span:last-child').textContent = live ? 'en vivo' : (label || 'desconectado');
  }

  // ======================== CHATS ========================
  function reloadChats() {
    return fetch('/api/admin/chats', { headers: authHeader() })
      .then(r => r.json())
      .then(({ chats }) => {
  ...
  function openChat(id) {
    ...
    fetch(`/api/admin/chats/${id}/messages`, { headers: authHeader() })
      .then(r => r.json())
      .then(({ messages }) => {
        threadEl.innerHTML = '';
        (messages || []).forEach(appendMessage);
  ...
  // Enviar respuesta vía HTTP (no depende de Socket.io)
  composer.addEventListener('submit', async (e) => {
    ...
    try {
      await fetch(`/api/admin/chats/${STATE.activeChatId}/message`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...authHeader() },
        body: JSON.stringify({ text }),
      });
      replyInput.value = '';
      // Si Socket.io no está, recargamos manualmente
      if (!STATE.socket || !STATE.socket.connected) {
        setTimeout(() => {
          fetch(`/api/admin/chats/${STATE.activeChatId}/messages`, { headers: authHeader() })
            .then(r => r.json())
            .then(({ messages }) => {
              threadEl.innerHTML = '';
              (messages || []).forEach(appendMessage);
  ...
  closeBtn.addEventListener('click', async () => {
    ...
    await fetch(`/api/admin/chats/${STATE.activeChatId}/close`, {
  ...
  // ======================== SETTINGS ========================
  const MSG_KEYS = ['greeting1','greeting2','afterName','askUser','noAccount','noAccountAck',
  ...
  function loadSettings() {
    fetch('/api/admin/settings', { headers: authHeader() })
      ...
      .then(({ settings, defaults }) => {
        STATE.settings = settings; STATE.defaults = defaults;
  ...
  settingsSave.addEventListener('click', () => {
    ...
    fetch('/api/admin/settings', {
      ...
      .then(({ ok, settings: next, error }) => {
        ...
        STATE.settings = next;
        settingsInfo.textContent = '✓ Guardado. Los cambios ya están en el chat.';
      })
      .catch(err => {
        settingsSave.disabled = false;
  ...
  settingsReset.addEventListener('click', () => {
    ...
    settingsInfo.textContent = 'Tocá "Guardar cambios" para aplicarlos.';
    settingsInfo.classList.remove('err');
  });
